...
# window size
# window_width = (top_button_width_pref) * num_top
# window_height = (top_button_height_pref) + (left_button_height_pref) * num_left
window_width = int((top_button_width_pref) * num_top)
window_height = int((top_button_height_pref) + (left_button_height_pref) * num_left/2)  
...
if show_window:
    ...
    for j in range(num_top, num_top + num_left):
        if num_top <= 20: 
            image3 = Image.open("../images/" + str(j+1-num_top) + ".png")
...
def main():
    ...
    for i in range(len(verticalflow_result_index)):
        for j in range(len(verticalflow_result_index[i])):
            index = verticalflow_result_index[i][j] + num_top
