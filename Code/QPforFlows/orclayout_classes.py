...
class Flow(ORCLayout, ABC):
    ...
    def solve(self):
        ...
        optimizer = cvx.Problem(obj, self.constraints)
        optimizer.solve()
        self.loss = optimizer.value + self.flow_loss
...
class HorizontalFlow(Flow):
    ...
    def constraint_spec(self):
        ...
        # add all the variables, constraints, objectives of widgets in the sublayout
        # to this sublayout
        # minimize the distance between preferred
        # sizes of widgets and the calculated sizes of widgets
        # and limit sizes between max size and min size
        for widget in self.widget_list:
            if not self.copied_tree:
                widget.constraint_spec()
            self.variables.update(widget.variables)
            self.constraints += widget.constraints
            self.objectives += widget.objectives
        ...
        if self.copied_tree:
            ...
            if self.connect_to == None:
                self.min_row = get_num_rows(self.width, self.min_w_list)
            ...
            for i in range(self.pref_row - 1, self.min_row - 1, -1):
                if not self.balanced or i in self.factors:
        ...
        if self.connect_to != None:
            ...
            entire_widget_list = self.widget_list + self.connect_to.widget_list
            ...
            self.widget_name_list = []
            for w in entire_widget_list[:index]:
                self.widget_name_list.append(w.name)
            ...
            self.connect_to.widget_name_list = []
            for w in entire_widget_list[index:]:
                self.connect_to.widget_name_list.append(w.name)
            for widget in entire_widget_list[:index]:
                self.variables.update(widget.variables)
                self.constraints += widget.constraints
                self.objectives += widget.objectives

        if not hasattr(self, 'widget_name_list'):
            self.widget_name_list = []
        ...
        # for each widget in the flow sublayout, create boundary constraints
        for r in range(len(self.result_index)):
            for k in range(len(self.result_index[r])):
                i = self.result_index[r][k]
                
                # for the first widget in each row, the left boundary is the same as
                # the left boundary of sublayout
                if k == 0:
                    left_constraint = (self.variables[self.widget_name_list[i] + "_l"] \
                                       == self.variables[self.name + "_l"])
                    if r != 0:
                        # the top boundary of the first widget in each row except the first
                        # widget in the first row is the same as the bottom boundary of the
                        # previous row
                        top_constraint = (self.variables[self.widget_name_list[i] + "_t"] \
                                          == self.variables[self.widget_name_list[i-1] + "_b"])
                    else:
                        # the top boundary of the first widget in the first row is the same as
                        # the top boundary of the sublayout
                        top_constraint = (self.variables[self.widget_name_list[i] + "_t"] \
                                          == self.variables[self.name + "_t"])
                
                # for the other widgets in each row, the left boundary is the same as
                # the right boundary of the previous widget
                # the top boundary is the same as the top boundary of the previous one
                else:
                    left_constraint = (self.variables[self.widget_name_list[i] + "_l"] \
                                       == self.variables[self.widget_name_list[i-1] + "_r"])
                    top_constraint = (self.variables[self.widget_name_list[i] + "_t"] \
                                     == self.variables[self.widget_name_list[i-1] + "_t"])
                                     
                    # the bottom boundary of the widgets in each row except the first
                    # widget in the each row is the same as the bottom boundary of the
                    # previous widget
                    bottom_constraint = (self.variables[self.widget_name_list[i] + "_b"] \
                                      == self.variables[self.widget_name_list[i-1] + "_b"])
                    self.constraints += [bottom_constraint]

                # the right boundary of the last widget in each row is the same as 
                # the right boundary of the sublayout
                if k == len(self.result_index[r]) - 1:
                    right_constraint = (self.variables[self.widget_name_list[i] + "_r"] \
                                       == self.variables[self.name + "_r"])
                    self.constraints += [right_constraint]
                
                self.constraints += [left_constraint, top_constraint]
        ...
        if self.fixed_boundary:
            delta = cvx.Variable()
            self.variables[self.name + "_bottom_delta"] = delta
            bottom_constraint = (self.variables[self.name + "_b"] \
                            - self.variables[self.widget_name_list[i-1] + "_b"] + delta == 0)
            self.constraints += [bottom_constraint]
            bottom_objective = self.weight * cvx.square(delta)
            self.objectives += [bottom_objective]
...
class FlowAroundFix(Flow):
    ...
    def constraint_spec(self):
        ...
        ########## Upper Area ##########
        # for each widget in the flow sublayout, create boundary constraints
        for r in range(len(self.result_index_upper)):
            for k in range(len(self.result_index_upper[r])):
                i = self.result_index_upper[r][k]
                
                # for the first widget in each row, the left boundary is the same as
                # the left boundary of sublayout
                if k == 0:
                    left_constraint = (self.variables[self.widget_list[i].name + "_l"] \
                                       == self.variables[self.name + "_l"])
                    if r != 0:
                        # the top boundary of the first widget in each row except the first
                        # widget in the first row is the same as the bottom boundary of the
                        # previous row
                        top_constraint = (self.variables[self.widget_list[i].name + "_t"] \
                                          == self.variables[self.widget_list[i-1].name + "_b"])
                    else:
                        # the top boundary of the first widget in the first row is the same as
                        # the top boundary of the sublayout
                        top_constraint = (self.variables[self.widget_list[i].name + "_t"] \
                ...
                # the top boundary is the same as the top boundary of the previous one
                    ...
                    left_constraint = (self.variables[self.widget_list[i].name + "_l"] \
                                       == self.variables[self.widget_list[i-1].name + "_r"])
                    top_constraint = (self.variables[self.widget_list[i].name + "_t"] \
                                     == self.variables[self.widget_list[i-1].name + "_t"])
                                     
                    # the bottom boundary of the widgets in each row except the first
                    # widget in the each row is the same as the bottom boundary of the
                    # previous widget
                    bottom_constraint = (self.variables[self.widget_list[i].name + "_b"] \
                                      == self.variables[self.widget_list[i-1].name + "_b"])
                    self.constraints += [bottom_constraint]
                ...
                # the right boundary of the last widget in each row is the same as 
                # the right boundary of the sublayout
                if k == len(self.result_index_upper[r]) - 1:
                    right_constraint = (self.variables[self.widget_list[i].name + "_r"] \
                                       == self.variables[self.name + "_r"])
                    self.constraints += [right_constraint]

        # the bottom boundary of the last widget is the same as the top boundary
        # of the fixed area
        bottom_constraint = (self.variables[self.widget_list[i].name + "_b"] \
                                       == self.fixed_top)
        self.constraints += [bottom_constraint]
        ...
        ########## Middle Area ##########
        # for each widget in the flow sublayout, create boundary constraints
        for r in range(len(self.result_index_middle)):

            # the area to the left of the fixed area
            for k in range(len(self.result_index_middle[r][0])):
                i = self.result_index_middle[r][0][k] + self.end_index_upper
                ...
                if k == 0:
                    ...
                    # the top boundary of the first widget in each row
                    # is the same as the bottom boundary of the previous row
                    top_constraint = (self.variables[self.widget_list[i].name + "_t"] \
                                      == self.variables[self.widget_list[i-1].name + "_b"])
                ...
                # the top boundary is the same as the top boundary of the previous one
                    ...
                    # the bottom boundary of the widgets in each row except the first
                    # widget in each row is the same as the bottom boundary of the
                    # previous widget
                    bottom_constraint = (self.variables[self.widget_list[i].name + "_b"] \
                                      == self.variables[self.widget_list[i-1].name + "_b"])
                    self.constraints += [bottom_constraint]
                ...
                # the right boundary of the last widget in each row is the same as 
                # the left boundary of the fixed area
                if k == len(self.result_index_middle[r][0]) - 1:
                    right_constraint = (self.variables[self.widget_list[i].name + "_r"] \
                                       == self.fixed_left)
                    self.constraints += [right_constraint]

            # the area to the right of the fixed area
            for k in range(len(self.result_index_middle[r][1])):
                i = self.result_index_middle[r][1][k] + self.end_index_upper
                
                # for the first widget in each row, the left boundary is the same as
                # the right boundary of the fixed area
                if k == 0:
                    left_constraint = (self.variables[self.widget_list[i].name + "_l"] \
                                       == self.fixed_right)
                ...
                # the right boundary of the last widget in each row is the same as 
                # the right boundary of the sublayout
                if k == len(self.result_index_middle[r][1]) - 1:
                    right_constraint = (self.variables[self.widget_list[i].name + "_r"] \
                                       == self.variables[self.name + "_r"])
                    self.constraints += [right_constraint]
        ...
        ########## Lower Area ##########
        # for each widget in the flow sublayout, create boundary constraints
        for r in range(len(self.result_index_lower)):
            for k in range(len(self.result_index_lower[r])):
                i = self.result_index_lower[r][k] + self.end_index_middle
                ...
                # the right boundary of the last widget in each row is the same as 
                # the right boundary of the sublayout
                if k == len(self.result_index_lower[r]) - 1:
                    right_constraint = (self.variables[self.widget_list[i].name + "_r"] \
                                       == self.variables[self.name + "_r"])
                    self.constraints += [right_constraint]
...
class VerticalFlow(Flow):
    ...
    def constraint_spec(self):
        ...
        # for each widget in the flow sublayout, create boundary constraints
        for c in range(len(self.result_index)):
            for k in range(len(self.result_index[c])):
                i = self.result_index[c][k]
           
                # for the first widget in each column, the top boundary is the same as
                # the top boundary of sublayout
                if k == 0:
                    top_constraint = (self.variables[self.widget_name_list[i] + "_t"] \
                                      == self.variables[self.name + "_t"])
                    if c != 0:
                      # the left boundary of the first widget in each column except the first
                      # widget in the first column is the same as the left boundary of the
                      # previous column
                      left_constraint = (self.variables[self.widget_name_list[i] + "_l"] \
                                        == self.variables[self.widget_name_list[i-1] + "_r"])
                    else:
                      # the left boundary of the first widget in the first column is the same as
                      # the left boundary of the sublayout
                      left_constraint = (self.variables[self.widget_name_list[i] + "_l"] \
                                        == self.variables[self.name + "_l"])
                
                # for the other widgets in each column, the top boundary is the same as
                # the bottom boundary of the previous widget
                # the left boundary is the same as the left boundary of the previous one
                else:
                    top_constraint = (self.variables[self.widget_name_list[i] + "_t"] \
                                     == self.variables[self.widget_name_list[i-1] + "_b"])
                    left_constraint = (self.variables[self.widget_name_list[i] + "_l"] \
                                       == self.variables[self.widget_name_list[i-1] + "_l"])
                
                    # the right boundary of the widgets in each row except the first
                    # widget in the each column is the same as the right boundary of the
                    # previous widget
                    right_constraint = (self.variables[self.widget_name_list[i] + "_r"] \
                                     == self.variables[self.widget_name_list[i-1] + "_r"])
                    self.constraints += [right_constraint]

                # the bottom boundary of the last widget in each col is the same as 
                # the bottom boundary of the sublayout
                if k == len(self.result_index[c]) - 1:
                    bottom_constraint = (self.variables[self.widget_name_list[i] + "_b"] \
                                       == self.variables[self.name + "_b"])
                    self.constraints += [bottom_constraint]
        ...
        if self.fixed_boundary:
            delta = cvx.Variable()
            self.variables[self.name + "_right_delta"] = delta
            right_constraint = (self.variables[self.name + "_r"] \
                                 - self.variables[self.widget_name_list[i-1] + "_r"] + delta == 0)
            self.constraints += [right_constraint]
            tight_objective = self.weight * cvx.square(delta)
            self.objectives += [right_objective]
